...
Make Emacs substitute the selected text when typing. It is something I always
...
(global-set-key (kbd "C-x \\ r") 'reload-emacs-configuration)
(global-set-key (kbd "C-x \\ o") 'open-emacs-configuration)
#+END_SRC
...
are available in MELPA. We also use the ~NO-CONFIRM~ flag when loading, since
the file tangling probably makes the code be not in order for

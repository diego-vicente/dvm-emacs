...
(use-package elpy
  :ensure t
  :init
  (setq elpy-env-path "/home/dvicente/Utilities/anaconda3/envs/elpy-tools")
  (setenv "PYTHONPATH" (concat elpy-env-path "/lib/python3.7/site-packages/"))
  :config
  (elpy-enable)
  (add-hook 'elpy-mode-hook (lambda () (highlight-indentation-mode -1))))
#+end_src

*** Support for =python-language-server=

Remember to install =pyls= in the base environment so that it is available.

#+BEGIN_SRC shell :tangle no
conda install -c conda-forge python-language-server
#+END_SRC

This ensures that the base Anaconda environment is the one used in Emacs.

#+BEGIN_SRC emacs-lisp :results silent
(add-to-list 'exec-path "/home/dvicente/Utilities/anaconda3/bin")
#+END_SRC
...
*** Include Jinja2 template support
...
It was about time to give it a shot, wasn't it?
...
(use-package cider
  ...
  :bind (:map cider-mode-map
              ...
              ("C-c p u" . cider-run)
              ("C-c p P" . cider-test-run-project-tests)))
#+END_SRC

...
This function access the theme variable to get the colors easily in case I have
to use them in other customizations.

#+BEGIN_SRC emacs-lisp
(defun get-base16-color (base16-property)
  "Get the color from OceanicNext theme list"
  (plist-get base16-oceanicnext-colors base16-property))
#+END_SRC
...
Below, you can see all the faces customizations I apply to ~neotree~ for better
blending into the rest of my color scheme:
...
(base16-set-faces
     ...
     '(
       ...
       (neo-banner-face      :foreground base09)
       (neo-header-face      :foreground base04)
       (neo-file-link-face   :foreground base06)
       (neo-root-dir-face    :foreground base0E)
))
#+END_SRC
...
(persp-mode)
;; Not working with use-package :bind configuration
(define-key projectile-mode-map (kbd "C-c p k") 'kill-current-perspective)
#+END_SRC
...
(setq eshell-prompt-function
      (lambda ()
          (concat
           (propertize "┌─[" 
		       'face '(:foreground (get-base16-color :base0D)))
           (propertize (user-login-name) 
		       'face '(:foreground (get-base16-color :base0E)))
           (propertize "@" 
		       'face '(:foreground (get-base16-color :base0D)))
           (propertize (system-name) 
		       'face '(:foreground (get-base16-color :base0C)))
           (propertize "]──[" 
		       'face '(:foreground (get-base16-color :base0D)))
           (propertize (format-time-string "%H:%M" (current-time))
                       'face '(:foreground (get-base16-color :base05)))
           (propertize "]──[" 
		       'face '(:foreground (get-base16-color :base0D)))
           (propertize (concat (eshell/pwd)) 
		       'face '(:foreground (get-base16-color :base09)))
           (propertize "]\n" 
		       'face '(:foreground (get-base16-color :base0D)))
           (propertize "└─>" 
		       'face '(:foreground (get-base16-color :base0D)))

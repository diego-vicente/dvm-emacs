...
(use-package flyspell
  ...
  (defun change-dictionary-spanish ()
	(interactive)
	(ispell-change-dictionary "español"))
...
(use-package easy-escape
  :ensure t
  :hook ((lisp-mode emacs-lisp-mode) . easy-escape-minor))
#+END_SRC
...
(use-package ox-publish
  ...
  ;; To convert links in files from ".html" to "/"
  (defun convert-html-link (link)
	"Turn an .html link into a / link."
	(let ((placeholder-extension (concat "." org-html-extension)))
	  (replace-regexp-in-string placeholder-extension "" link)))

  (defun filter-local-links (link backend info)
	"Filter that converts all the .html links to /."
	(if (org-export-derived-backend-p backend 'html)
		(convert-html-link link)))

  ;; To fix the local placeholder used for link conversion
  (defun replace-local-org-file (file)
	"Returns the path to a file with the html extension instead of the placeholder."
	(replace-regexp-in-string org-html-extension "html" file))

  (defun rename-exported-file (original exported)
	"Rename exported file to remove the local placeholder extension."
	(when (string-match-p org-html-extension exported)
	  (let ((new-name(replace-local-org-file exported)))
		(if (file-exists-p new-name)
			(delete-file new-name))
		(rename-file exported new-name))))
  ...
  (add-hook 'org-publish-after-publishing-hook 'rename-exported-file))
#+END_SRC
